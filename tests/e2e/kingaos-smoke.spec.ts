...
test("super_admin 打开新建客户并看到分步骤、地理选择和多联系人", async ({ page }) => {
  ...
  await page.getByLabel(/公司名称/).fill("Playwright 测试客户");
...
test("新建客户页显示重复客户审核入口和申请原因", async ({ page }) => {
  await login(page, "export.a@kingaos.local", "Kingaos@123456");
  await page.goto("/export/customers/new");
  await expect(page.getByRole("heading", { name: "新建客户" })).toBeVisible();
  await page.getByLabel(/公司名称/).fill(`重复检测 ${Date.now()}`);
  for (let index = 0; index < 5; index += 1) {
    await page.getByRole("button", { name: "下一步" }).click();
  }
  await expect(page.getByLabel("重复客户审核申请原因")).toBeVisible();
  await page.goto("/export/customers");
  await expect(page.getByRole("link", { name: "重复客户审核" })).toBeVisible();
});
