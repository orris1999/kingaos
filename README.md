...
- 字段类型在 UI 中显示中文，内部仍保存 `text` / `textarea` / `number` / `date` / `select` / `multiselect` / `boolean` / `url` / `attachment`。
- 管理员可以修改自定义字段类型；系统字段类型默认锁定，避免影响客户档案基础结构。
- 字段配置支持多选、超链接和附件字段；下拉 / 多选选项可维护内部说明，内部说明只在填写页面显示，不进入详情页或未来对外客户信息卡。
- 客户类型支持多选；旧 `Customer.customerType` 字段继续保留兼容，新数据使用 `Customer.customerTypes` 保存多个业务属性。
- 附件字段复用 `CustomerAttachment` 和 OSS 上传能力，`customFields` 只保存附件 ID 引用，不保存文件二进制、base64 或临时 uploadUrl。
- 附件字段在表单中使用紧凑控件，默认只显示附件数量、列表和“上传文件 / 添加链接”入口；不会在联系人信息页默认展开完整上传表单。
- 通用客户附件只展示没有字段归属的附件，并集中在附件区域维护；字段型附件按字段分组展示。
- 客户来源按自定义字段配置管理，可由管理员调整类型、分组、必填和选项；旧 `Customer.source` 字段保留兼容历史数据。
- 页面上统一把客户主体显示为“公司名称”，内部字段仍保留 `Customer.name`；公司信息中的旧 `companyName` 字段只保留兼容，表单不再重复填写。
- 客户列表使用横向滚动表格和固定最小宽度，避免表头和内容被压成逐字换行。
- 客户类型多选使用紧凑下拉控件，选择后以标签显示，不在基础信息页直接铺开全部选项。
- 公司名称默认不允许重复；系统仍基于内部 `Customer.name` 做规范化判重，加点、加空格、大小写变化、全角半角变化不能绕过。
- 重复客户必须提交业务经理 / 管理员审核，审核通过后才允许例外建档，并写入 AuditLog。
- 客户档案可选择一个财务维护的官方默认收款方案；业务员只能选择，不能手填或修改银行账号明细。
- 默认收款方案在客户列表和表单中紧凑展示；账号详情标记为“财务维护，业务只读”，详细银行信息折叠查看。
- 财务停用官方收款账号后，系统会显示受影响客户；已引用客户保留历史引用并显示停用提醒，新客户不能再选择该账号。
- 客户列表支持按默认收款方案状态筛选：未设置、有效、已停用。
- 客户档案关键字段支持修改历史，记录修改人、修改时间、原值和新值。
- 修改历史跟随客户查看权限；客户转给新负责人后，新负责人可以看到该客户过去历史，出口部经理可以看到出口部客户历史。
- 管理员后台提供只读版本更新日志 `/admin/changelog`，更新记录由 `docs/CHANGELOG.md` 和 `lib/honoa/shared/release-notes.ts` 双轨维护，不写入数据库。
- 以后每轮代码、页面、文档、配置、测试或部署流程变更，都必须同步更新版本日志。
- KingaOS 采用统一 B2B SaaS 视觉基线：深绿色主色、中性色背景、白色卡片、细边框、柔和阴影、统一圆角、统一间距和清晰按钮层级。
...
- 财务停用收款账号时，系统只做影响提醒和引用客户列表，不自动清空、不自动替换客户默认收款方案；批量替换账号不在当前范围。
- 客户字段修改历史保存到 `CustomerFieldChangeHistory`，只用于内部客户档案追溯，不对外展示。
- 字段历史记录默认收款方案时只保存方案名称、账号编号和收款账号 ID，不保存完整银行账号全文。
- `已归档` 在 UI 中显示为更易理解的 `资料已完善`，表示资料基本完整，不代表客户停止合作。
- 必填字段使用红色 `*`、浅色底和“必填”标识；只读 / 系统生成字段使用灰底和“只读”或“系统生成”提示。
- 客户新建 / 编辑页按步骤卡片组织，基础信息采用两列布局；字段 label 统一位于输入控件上方。
- 新建客户时附件需要保存客户后上传；保存后可在客户详情 / 编辑页面添加名片、营业执照、聊天记录、报价资料等附件。
- 新建客户时如果遇到附件字段，也只提示保存后上传，不做本地缓存、localStorage 暂存或 ECS 临时文件。
- 规范化公司名称保存到 `normalizedCustomerName`，用于服务端重复客户检测。
...
- 客户附件优先使用 OSS 上传；附件字段也支持添加外部附件链接，历史外部链接附件继续兼容展示和下载。
...
- 当前不做字段回滚 / 版本恢复，不做联系人字段级历史；附件字段仅记录附件数量 / 引用变化，不记录文件内容或临时 URL，联系人和通用附件操作继续使用操作记录。
