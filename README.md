...
- 国家 / 地区 → 州 / 省 / 地区 → 城市联动选择，并保留手动输入城市 fallback。
...
- 国家、州省、城市同时保存 code 和 name；旧 `country` / `city` 字段继续保留兼容。
...
国家 / 州省 / 城市数据通过服务端 API 按需读取，不在 client component 中直接 import 全量数据，也不把全量城市数据一次性打进前端 bundle。
