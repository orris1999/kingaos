...
Quote Task 003C 增加了本地 CLI：`npm run quote-source:dry-run -- --file "/path/to/file.xlsx"`。该 CLI 只读取显式指定 Excel 文件的结构信息，并复用 003B 的 matcher：

- 只读取 workbook metadata、sheet 名称、used range 和表头候选。
- 不默认扫描目录，不上传文件，不写数据库，不导入报价表。
- 检测到成本 / 报价列时只输出布尔信号，不输出任何金额或产品行明细。
