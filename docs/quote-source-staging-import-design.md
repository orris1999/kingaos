...
## Quote Task 009R-Fix candidate amount authenticated UAT path

009R production UAT 的失败点是 authenticated session：candidate amount import route 没有拿到已登录 `super_admin` 的 session cookie，返回 `401 未登录`。这说明 route 的认证边界有效，但 UAT 不能用无 cookie curl 直接调用。

009R-Fix 的正确执行路径：

1. `super_admin` 先登录 KingaOS。
2. 进入 `/finance/quote-source-staging/[batchId]`。
3. 页面在 `KINGA_ENABLE_FINANCE_QUOTE_CANDIDATE_AMOUNT_IMPORT=true` 时显示候选金额导入表单。
4. 表单由浏览器发起 same-origin request，自动携带 httpOnly cookie。
5. 前端不传 `actorUserId`，服务端只从 current-user session 识别 actor。
6. route 继续拒绝未登录 request；无 cookie curl 仍应返回 `401`。

业务边界不变：

1. 只允许 `export_usd` / `domestic_cny`，拒绝 `unknown`。
2. 只处理 `finance_confirmed + export_draft_candidate + candidate` rows。
3. `needs_manual_review` rows 不导入候选金额。
4. `QuoteCandidateAmount.visibility` 默认并保持 `finance_only`。
5. 候选金额不是 `FinanceApprovedPrice`，不能发客户。
6. 不生成 `QuoteDraft` / `QuoteDraftLine`，不生成正式报价。

009R-Fix 不执行 production import，不修改 ECS `.env`。真正 production UAT 留到 009R-Retry。
