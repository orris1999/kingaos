...
`npm run db:seed` 是生产安全 seed：只维护权限字典和缺失的系统字段定义，不创建用户、不修改用户、不修改客户，也不创建财务官方收款账号。

财务官方收款账号属于生产主数据，必须由有权限的财务 / 管理员在 `/finance/receipt-accounts` 手工维护。业务员只能在客户档案选择有效官方收款方案，不能手填银行账号。未来合同使用收款账号时必须保存合同快照；当前部署不包含合同模块。

财务停用官方收款账号后，系统会在账号详情页显示当前引用该账号的客户摘要，并在停用前提示受影响客户数量。客户档案继续保留历史引用并显示风险提醒，系统不会自动清空或自动替换客户默认收款方案。任何批量替换账号都必须未来单独设计 dry-run、审批和审计流程。
