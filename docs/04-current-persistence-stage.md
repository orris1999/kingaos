...
- 客户列表使用横向滚动表格和固定最小宽度，避免表头、操作列和多选标签被压成逐字换行。
- 新建 / 编辑客户按步骤卡片组织，基础信息采用两列布局；客户类型多选使用紧凑下拉控件，选择结果以标签显示。
- 默认收款方案在列表和表单中紧凑展示，银行账号详情折叠查看，并明确标记“财务维护，业务只读”。
...
- 附件字段在新建 / 编辑表单中使用紧凑控件：默认只显示附件数量、附件列表和“上传文件 / 添加链接”入口，点击后才展开具体表单。
- 通用客户附件区只展示 `fieldKey` 为空的通用附件；字段型附件按字段归属展示，避免联系人信息页重复出现完整上传表单。
...
- 财务收款账号详情页显示当前使用该账号的客户列表；停用账号前会提示受影响客户数量。
- 客户列表支持按默认收款方案状态筛选：未设置、有效、已停用。
- 系统不会因为财务停用账号而自动清空或自动替换 `Customer.defaultReceiptAccountId`；批量替换必须未来单独设计 dry-run、审批和审计。
