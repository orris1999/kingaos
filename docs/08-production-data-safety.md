...
财务官方收款账号只能通过应用内权限页面维护，不允许通过 seed、bootstrap、backfill 或部署脚本自动创建示例账号。停用账号只做状态变更，不物理删除历史引用；已引用客户需要在页面提示重新选择有效账号。系统可以展示停用账号影响的客户数量和有限客户摘要，但不能自动清空或自动替换客户默认收款方案。批量替换账号不属于当前阶段，未来如需实现必须先有 dry-run、审批、备份和审计。
