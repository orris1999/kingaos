...
Quote Task 009K-Fix 修复 Finance staging confirmation action path：

- 009K production UAT 未能完成 visibility promotion，最可能原因是 confirmation domain 仍被默认 production write guard 拦截。
- 修复后默认 production guard 继续保留，只为 Finance confirmation action 增加专用受控 production write reason。
- `KINGA_ENABLE_FINANCE_STAGING_CONFIRM` 仍默认关闭，且不使用 `NEXT_PUBLIC_`。
- 仍只允许 `super_admin`。
- `dryRunDecisionStatus = manual_review_required` 不代表失败；它表示需要人工确认，`super_admin` 可在人工确认后推进。
- 确认策略只允许 `strict_candidate_only`，不允许 `include_manual_review`。
- 合格 `candidate` rows 可从 `finance_only` 提升为 `export_draft_candidate`。
- `needs_manual_review` / `addon_only` / `blocked` / `ignored` / `missing` / `requires_finance_review` 仍不会给出口部消费。
- `export_draft_candidate` 仍不是正式报价，`finance_confirmed` 仍不是 FinanceApprovedPrice。
- 009K-Fix 不执行 production confirm；production retry 留到 009K-Retry。
- row visibility 默认 `finance_only`，`candidate` 不等于 `export_draft_candidate`。
- 本轮不新增 API route、server action、Prisma schema 或 migration。
- 本轮不写 production，不生成报价草稿，不生成正式报价。

Quote Task 009I 补充 feature-gated row import action / route 的 local / test DB 验证：

- 新增 `KINGA_ENABLE_FINANCE_QUOTE_SOURCE_ROW_IMPORT`，缺失 / `false` 默认关闭。
- 第一版仍只支持 `condenser-cost-2026 / 冷凝器`。
- action 只能由 `super_admin` 对 `dry_run_passed` batch 执行。
- 通过 `stagingBatchId` 找到对应 `QuoteSourceUpload`，再调用 009H parser + mapper。
- 创建的 rows 默认 `finance_only`，不会自动成为 `export_draft_candidate`。
...
- `export_usd` 使用 `2026.5.11出口成本报价` 作为外销 / 美金 / 有退税场景候选来源。
- `domestic_cny` 使用 `2026.5.11出口部内销成本报价` 作为内销 / 人民币场景候选来源。
- `unknown` 不自动选择候选金额。
...
Quote Task 009O 新增候选金额 local/test importer，但仍不新增 production 可见能力：

- 第一版只支持 `condenser-cost-2026 / 冷凝器`。
- `export_usd` 读取 `2026.5.11出口成本报价`，币种 `USD`。
- `domestic_cny` 读取 `2026.5.11出口部内销成本报价`，币种 `CNY`。
- `unknown` 不自动导入候选金额，旧日期列不作为默认候选。
- importer 只在 local/test DB 创建 `QuoteCandidateAmount`，默认 `finance_only` / `not_finance_approved`。
...
Quote Task 009Q 修复 candidate amount import controlled production guard，但不执行 production import：

- 保留 repository 默认 production guard，缺少 controlled option 仍拒绝 production 写入。
- controlled reason 固定为 `finance_quote_candidate_amount_import_uat`。
- 只有 candidate amount import action 在完成全部业务校验后传入 controlled option。
- action 必须校验 feature flag、`super_admin`、`finance_confirmed` batch、`export_draft_candidate` rows、upload/storageKey 和 tradeMode。
- action 只允许 `export_usd` / `domestic_cny`，拒绝 `unknown`。
