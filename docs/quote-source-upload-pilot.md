...
- `POST /api/finance/quote-source-upload/[uploadId]/confirm-dry-run`
...
009E 新增 dry-run 结果确认 route，但默认 production 关闭：

- 服务端 feature flag：`KINGA_ENABLE_FINANCE_QUOTE_SOURCE_DRY_RUN_CONFIRM`
- 缺失或 `false` 时视为关闭。
- 不使用 `NEXT_PUBLIC_`，不把 flag 暴露给前端。
- flag 关闭时，上传记录列表的确认按钮 disabled，并显示“dry-run 确认暂未开放”。
- flag 开启后，第一版仍只允许 `super_admin` 对 `uploadStatus = uploaded` 且 `dryRunStatus = completed` 的记录执行确认。
...
## Dry-run confirmation metadata

Quote Task 009E 在 `QuoteSourceUpload` 上增加 additive 确认 metadata 字段：

- `stagingBatchId`
- `dryRunConfirmedAt`
- `dryRunConfirmedByUserId`
- `dryRunConfirmedByName`

dry-run 确认只做一件事：把已完成的结构识别结果确认进入 `QuoteSourceStagingBatch` metadata。它仍然不读取 Excel 行、不保存价格、不保存 KJ 行 / OEM 行，也不生成报价草稿或正式报价。
...
- `dryRunCategory` 存在
- `dryRunSummary` 存在
- 当前 upload 尚未关联 `stagingBatchId`

确认后：

- 创建 `QuoteSourceStagingBatch` metadata。
- `submittedByRole = finance`
- `consumerDepartment = export`
- `status = dry_run_passed`，或按 dry-run decision 映射为对应 batch status。
- 写入 `stagingBatchId` 和确认人 / 确认时间。
- 防止同一 upload 重复确认生成多个 batch。
...
2. Finance dry-run 确认进入 staging batch metadata。009E 已实现 feature-gated batch-only confirmation，production 默认关闭。
3. Finance staging rows 生成。
4. Finance confirmation。
5. Export 只读消费 `finance_confirmed + export_draft_candidate` 脱敏候选。
6. FinancePricing 接入后才可能形成正式价格和正式报价。
